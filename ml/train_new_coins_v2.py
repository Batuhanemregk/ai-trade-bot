"""
Training script for new coins using ONLY Binance or OKX data
Coins: HYPE, CRO, XLM, ZEC (XMR excluded - not on OKX)
"""
import sys
...
# Coins to train - ONLY using Binance or OKX
# XMR excluded because not available on OKX
COINS_TO_TRAIN = {
    'HYPE': 'okx',     # OKX swap available
    'CRO': 'okx',      # OKX swap available  
    'XLM': 'binance',  # Available on both, Binance has more history
    'ZEC': 'binance',  # Available on both, Binance has more history
}


async def collect_ohlcv_data(symbol: str, exchange_name: str = "binance", timeframe: str = "15m", months: int = 18) -> pd.DataFrame:
    """Collect OHLCV data from specified exchange (Binance or OKX only)."""
    import ccxt
    
    try:
        if exchange_name == 'okx':
            # OKX swap format for OHLCV
            exchange = ccxt.okx({'timeout': 30000})
            full_symbol = f"{symbol}/USDT:USDT"  # OKX swap
        else:
            # Binance spot
        ...
        logger.info(f"📊 Collecting {months}M of {timeframe} data for {symbol} from {exchange_name}...")
...
async def train_coin_model(symbol: str, exchange: str, n_trials: int = 50) -> Dict:
    """Train model for a single coin."""
    logger.info(f"\n{'='*60}")
    logger.info(f"🚀 Training model for {symbol} (data from {exchange})")
    logger.info(f"{'='*60}")
    ...
    try:
        # 1. Collect data
        df = await collect_ohlcv_data(symbol, exchange, "15m", 18)
        ...
        logger.info(f"📈 Holdout: AUC={metrics['auc']:.4f}, Acc={metrics['accuracy']:.4f}, Prec={metrics['precision']:.4f}")
...
async def main():
    print("="*60)
    print("ML MODEL TRAINING - HYPE, CRO, XLM, ZEC")
    print("Data sources: OKX (HYPE, CRO), Binance (XLM, ZEC)")
    print("="*60)
    
    results = {}
    
    for symbol, exchange in COINS_TO_TRAIN.items():
        print(f"\n>>> Starting {symbol} from {exchange}...")
        result = await train_coin_model(symbol, exchange, n_trials=50)
        results[symbol] = result
        
        if result['success']:
            print(f"✅ {symbol}: AUC={result['metrics']['auc']:.4f}")
        else:
    ...
    for symbol, result in results.items():
        if result['success']:
            m = result['metrics']
            print(f"{symbol}: AUC={m['auc']:.4f} | Acc={m['accuracy']:.4f} | Prec={m['precision']:.4f} | Rec={m['recall']:.4f}")
        else:
            print(f"{symbol}: FAILED - {result.get('error', 'Unknown')}")
    
    # Save results
    with open("logs/new_coins_training_results.json", 'w') as f:
        json.dump(results, f, indent=2, default=str)
    
    print(f"\nResults saved to logs/new_coins_training_results.json")
